...
class UserStatusRepository {
  public async getStatusByDomain({
    ...
    const userStatusList = await connectionOracle.raw<RetornoDbStatus[]>(`
      select
          unique u.vch_username as usuario,
          u2.vch_username as usuario_terminal,
          ssl.vch_useragent as useragent,
          d.vch_domain as dominio,
          ac.vch_originalto as destino,
      ...
      from
          tbl_sys_domain d,
          tbl_sys_user u
      FULL OUTER JOIN tbl_pbx_pbxuser pu
      ...
      FULL OUTER JOIN tbl_sys_user u2
          ON u2.int_user_key = pu2.int_user_key
      FULL OUTER JOIN tbl_pbx_address a2
          ON a2.int_pbxuser_key = pu2.int_pbxuser_key
      FULL OUTER JOIN tbl_pbx_activecall ac
          ON ac.int_address_key in (a.int_address_key, a2.int_address_key )
      FULL OUTER JOIN tbl_sys_sessionlog sl
          ON sl.int_user_key = u.int_user_key
      FULL OUTER JOIN tbl_sys_sipsessionlog ssl
          ON ssl.int_sessionlog_key = sl.int_sessionlog_key
      where
          d.vch_domain = '${domain}' and
          d.int_domain_key = u.int_domain_key and
          u.int_agentuser in (0, 7) and
          u.int_active = 1 and
          ssl.vch_useragent is not null
      order by
          u.vch_username
    `);
